...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.l1 = nn.Sequential(
            nn.Linear(100, 128 * self.init_size ** 2))  # l1函数进行Linear变换。线性变换的两个参数是变换前的维度，和变换之后的维度

        self.conv_blocks = nn.Sequential(  # nn.sequential{}是一个组成模型的壳子，用来容纳不同的操作
            # 标准化（数据归一化）输⼊特征X，使其平均值为0，⽅差为1。直观地说，这种标准化可以很好地与我们的优化器配合使⽤，因为它可以将参数的量级进⾏统⼀
    ...
    def forward(self, z):
        ...
        out = out.view(out.shape[0], 128, self.init_size,
                       self.init_size)  # view是维度变换函数，可以看到out数据变成了四维数据，第一个是batch_size(通过整个的代码，可明白),第二个是channel，第三,四是单张图片的长宽
